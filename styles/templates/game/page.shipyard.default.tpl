...
<div  onclick="return Dialog.info({$ID})" style=" width:172px; height:172px; background:url('{$dpath}gebaeude/{$ID}.gif'); background-size:100% 100%;"><div style="background:rgba(13, 16, 20, 0.95); height: 30px; border-bottom: 1px solid #000; padding-left:2px; width:100%;"><a class="tooltip" data-tooltip-content="<div style='max-width:200px;'>{$LNG.shortDescription.{$ID}|replace:'"':'\''}<br><br>						{$LNG.bd_remaining|replace:'"':'\''}<br>
						...
						{$LNG.tech.{$ResType}|replace:'"':'\''}: <span style='font-weight:700'>{$ResCount|number:($ResType == 921)}</span><br>
		...
		<div style="; height:100px; text-align:right;" >
					...
					{if $Element.AlreadyBuild}<div style="text-align:center; border:1px solid #000; border-radius: 5px; height:34px; background-color: rgba(200, 0, 0, 1);"><span style="color:#fff">{$LNG.bd_protection_shield_only_one}</span></div>
					...
					<input type="submit" value="{$LNG.bd_build_ships}" class="build_submit" style="color:#fff; font-weight:bold; min-width:100%; margin:0px; text-align:center; border:1px solid #000; border-radius: 5px; height:19px; background-color:rgba(0, 0, 200, 1); vertical-align:center; "></div>
					{/if}
