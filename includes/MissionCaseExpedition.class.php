...
/*        if($GetEvent == 4)
		...
		switch($GetEvent)
		{
			case 1:
				...
				if($senderUser['class_explorer'] == 1)
				{
					$founded		= round(min($fleetCapacity,mt_rand($topPoints * 5, topPoints * 10)));
					$founded = $founded * (1+ (0.1*$senderUser['larrysluckycoin']));
				}
...
/*				if(!empty($Found[$GoldShip]))
            	...
            	switch($amountperradar)
            	{
                	...
                	case 1:
						...
						if($senderUser['class_explorer'] == 1)
						{
							$neededPoints = round($neededPoints / 4);
						}
                    	if($fleetPoints < $neededPoints)
                        {
							...
							if($senderUser['class_explorer'] == 1)
							{
								$neededPointsPerShip = round($neededPointsPerShip / 4);
							}
				...
				$Count = $Count * (1+(0.1*$senderUser['larrysluckycoin']));
				$Count = round($Count);
				$FoundShipMess   	.= '<br>'.$LNG['tech'][$GoldShip].': '.pretty_number($Count);
				foreach($reslist['fleet'] as $ID)
				{
